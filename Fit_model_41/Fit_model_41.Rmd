...
g_apap <- Y(g = observables_apap, f = x_apap, attach.input = F)
```
...
g_bic <- Y(observables_bic, x_bic, attach.input = F)
```
...
g_met <- Y(observables_met, x_met, attach.input = F)
```
...
compile(g_bicmet, e_bicmet, p_bicmet, p_bicmet_sigma_NA, cores = 3)
```
...
e_apapbicmet <- Y(c(errors_bic_sigma_NA, errors_met_sigma_NA), g_apapbicmet, modelname = "e_apapbicmet", compile = F)
```
...
basic_hypotheses.frame <- basic_hypotheses.frame %>% dMf_append_objfuns
