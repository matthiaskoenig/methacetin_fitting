...
# Start fits without Albert capsule 

The section above here we found out that the capsule data set is really different from all the others and kills everything
```{r load workspace with dMod.frames}
load("2018_03_31_02_21_workspace_with_dMod.frames.rda")
rbind(bicmet.frame, apapbicmet.frame) %>% .$obj %>% walk(loadDLL)
```
...
```{r}
no_alb_caps.frame <- apapbicmet.frame
no_alb_caps.frame <- no_alb_caps.frame %>% 
  mutate(data_0 = map(data_0, . %>% .[!((names(.) %>% str_detect("lbert")) & (names(.) %>% str_detect("caps")))] )) %>% 
  mutate(hypothesis= "apapbicmet_no_albert_capsule") %>% 
  mutate(prd_0 = map(seq_along(x), function(i) cfn(g[[i]], cfn(x[[i]],p_0[[i]]))),
         prd_sig = map(seq_along(x), function(i) cfn(g[[i]], cfn(x[[i]],p_sig[[i]])))) %>% 
  mutate(obj_0 = map(seq_along(x), function(i) normL2(data_0[[i]], prd_0[[i]])),
         obj_sig = map(seq_along(x), function(i) cf_normL2(data = data_sig[[i]], prd_sig[[i]], e = e[[i]]))) %>% 
...
```{r}
no_alb_caps.frame %>% mutate(test = map(seq_along(x), function(i) obj[[i]](getParameters(obj[[i]]) %>% are_names_of(0)))) %>% .$test
```

```{r}
no_alb_caps.frame <- no_alb_caps.frame %>% dMf_reduce()

no_alb_caps.frame[["pars"]] <- getParameters(no_alb_caps.frame[["obj"]][[1]]) %>% are_names_of(0) %>% list

# fit_no_alb_caps_broad_sampling <- runbg({
#   ncores <- detectFreeCores()
#   assign("ncores", ncores, pos = .GlobalEnv)
#   
#   no_alb_caps.frame %>%
...
#       mstrust(objfun = fit_obj, center = fit_pars, studyname = fit_studyname,
#               sd = 4,
#               blather = F, cores = ncores, fits = 30*ncores)
#     }))
# }, machine = paste0("knecht", 1:5), filename = paste0("tmp_",tpaste0("fit_no_alb_caps_broad_sampling")), input = "no_alb_caps.frame")
saveRDS(fit_no_alb_caps_broad_sampling, tpaste0("fit_job_no_alb_caps_broad_sampling.rds"))
```

```{r}
fit_no_alb_caps_broad_sampling <- readRDS("2018_04_01_09_21_fit_no_alb_caps_broad_sampling.rds")
no_alb_caps.frame$fits <- fit_no_alb_caps_broad_sampling$get() %>% map("fits") %>% map(1) %>%  Reduce(c.parlist,.) %>% list()
saveRDS(no_alb_caps.frame, tpaste0("fits_no_alb_caps_broad_sampling.rds"))
```


```{r}
no_alb_caps.frame$obj %>% walk(loadDLL)
no_alb_caps.frame <- no_alb_caps.frame %>% 
  dMf_expand_fits() %>% 
  dMf_append_plots()
```
...
```{r}
no_alb_caps.frame$parframes[[1]] %>% .[1:1000,] %>% plotValues(tol = 10 )
```


```{r}
no_alb_caps.frame$parframes %>% .[[1]] %>% .[c(1,42,50,80)] %>% plotPars(tol = 10)
```


```{r condition wise contributions}

condition_wise <- function(no_alb_caps.frame, index) {
  mypars <- no_alb_caps.frame$parframes[[1]] %>% as.parvec(index = index)
  
  myobj <- no_alb_caps.frame$obj_0[[1]]
  mycond <-  myobj %>% getConditions()
  
  condwise_contributions_0 <- map(mycond, function(cn) {
    # print(cn)
    myobj(pars = mypars, conditions = cn)
    }) %>% set_names(mycond)
  
  myobj <- no_alb_caps.frame$obj_sig[[1]]
  mycond <-  myobj %>% getConditions()
  
  condwise_contributions_sig <- map(mycond, function(cn) {
    # print(cn)
    myobj(pars = mypars, conditions = cn)
    }) %>% set_names(mycond)
  
  
  condwise_contributions <- c(condwise_contributions_0, condwise_contributions_sig)
  
  condwise_contributions %>% map_dbl("value")
}


myconditions <- sapply(c(1,42,80), function(i) condition_wise(no_alb_caps.frame, i))

myconditions %>% apply(1, function(i) (i-i[1]) %>% round(2)) 
```
...
checkout_hypothesis(apapbicmet.frame,  "apapbicmet", "abm")
checkout_hypothesis(profile.frame, profile.frame$hypothesis[1], "pr")
```
...
abmprd(seq(0,24,length.out = 200), prbest_parvec, deriv = F) %>% 
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/apap.png", width = 10, height = 6)
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/apap_zoom.png", width = 10, height = 6)
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/apap_only_Albert.png", width = 10, height = 6)
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/apap_only_Rawlins.png", width = 10, height = 6)
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/bic.png", width = 10, height = 6)
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/bic_cum_rec.png", width = 10, height = 6)
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/bic_DOB.png", width = 10, height = 6)
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/bic_P_CO2F.png", width = 10, height = 6)
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/met.png", width = 10, height = 6)

```

### exploration of problems in met
There is sth wrong in cum_rec_metc13, but its not the obesrvation function: The trick with init_PODOSE works, as the ratio between Abreatch and cum_rec stays constant.
Therefore I think, that either something is wrong with the parameter values or with Abreatch_co2c13, but not both.
But, if you uncomment the line "mutate(value = value * 0.1) %>%" two chunks below to scale down cum_rec by a constant, then you see, it's not a question of the parameters but of Abreath.
```{r}

controls(g_apapbicmet, name = "attach.input") <- T
(g_apapbicmet*abmx*p_apapbicmet)(seq(0,10,length.out = 10), prbest_parvec, deriv = F) %>% 
  wide2long() %>% 
  left_join(conditions_df) %>% 
  filter(drug_identifier == "met") %>% 
  # filter(! Study %>% str_detect("(2013)")) %>%
  filter(Study %>% str_detect("(Lala)")) %>%
  # filter(`Data set` %>% str_detect("D10")) %>% 
  # filter(name %in% names(observables_met)) %>% 
  filter(name%in%c("cum_rec_metc13", "Abreath_co2c13", "Alu_co2c13")) %>% 
  {.} %>% 
  spread(name,value) %>% 
  mutate(rat = Abreath_co2c13/cum_rec_metc13) %>% 
  mutate(rat2 = Alu_co2c13/cum_rec_metc13) %>% 
  {.}
```

```{r}
observables_met #%>% getSymbols()
mypars[observables_met %>% getSymbols()]
controls(g_apapbicmet, name = "attach.input") <- T
# 
# data_met %>% filter(study %>% str_detect("Lal"))
mypars <- pars_raw
mypars["PODOSE_metc13"] <- c(75)
# mypars["PODOSE_co2c13"] <- c(75)
# mypars["ti_co2c13"] <- 10
# mypars["ti_metc13"] <- 10

mypars <- c(mypars, init_PODOSE_metc13 = mypars["PODOSE_metc13"] %>% unname(), offset_bic = 0)
# mypars
(g_apapbicmet*abmx)(seq(0,20,length.out = 5), mypars, deriv = F) %>%
  # plot %>% 
{.}

# (abmx)(seq(0,10,length.out = 5), mypars, deriv = F) %>% .[[1]] %>% attributes()
```
...
g_test <- Y(observables_met, x_apapbicmet)

(g_test*abmx)(seq(0,10,length.out = 5), mypars, deriv = F) 




bla <- apapbicmet.frame$p_0[[1]](prbest_parvec) %>% str_subset_name("Lala") %>% .[[1]]
(g_test*abmx)(seq(0,10,length.out = 5), bla, deriv = F) 



mypars %>% unclass
bla %>% unclass 
wup <- ((bla %>% unclass() %>% sort_by_name())-(mypars%>% unclass() %>% sort_by_name())) %>% sort() %>% .[.!=0]

```


```{r}
controls(g_apapbicmet, name = "attach.input") <- F
# 
# data_met %>% filter(study %>% str_detect("Lal"))
mypars <- pars_raw
mypars["PODOSE_metc13"] <- c(75)
...
mypars["KBO_FIXCO2"] <- 1.3e+04
mypars["KBO_MAXCO2"] <- 0.08
mypars["KBO_RELCO2"] <- 4.88e+02
mypars["KLU_EXCO2"] <- 3.2

mypars["CYP1A2MET_Km_met"] <- exp(-16)

# bla %>% unclass %>% str_subset_name("CO2")
...
abmprd(seq(0,10,length.out = 200), prbest_parvec, deriv = F) %>% 
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/met_cum_rec_metc13.png", width = 10, height = 6)
...
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/met_mom_rec_metc13.png", width = 10, height = 6)
