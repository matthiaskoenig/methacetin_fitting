...
compile(g_apapbicmet, output = "g_apapbicmet")
...
data_apapbicmet <- mydata  %>% filter(!is.na(sigma)) %>% select(-simulation)
data_apapbicmet_sigma_NA <- mydata  %>% filter(is.na(sigma)) %>% select(-simulation)
...
pars_errors_apapbicmet <- c(errors_bic_sigma_NA, errors_met_sigma_NA) %>% 
  getSymbols %>% 
  str_subset("^s") %>% 
  are_names_of(1)
outer_pars_apapbicmet_sigma_NA <- sort(unique(c(outer_pars_apapbicmet, names(pars_errors_apapbicmet))))
...
compile(p_apapbicmet, p_apapbicmet_sigma_NA, cores = 3, output = "p_apapbicmet")
...
apapbicmet.frame <- dMod.frame(
  ...
  data_sig = list(as.datalist(data_apapbicmet_sigma_NA))
)
...
apapbicmet.frame <- apapbicmet.frame %>% 
  ...
  appendObj(obj = list(obj_data+ obj_data_sig)) %>% 
  mutate(pars = list(structure(rnorm(length(getParameters(obj))), names =  getParameters(obj))), 
...
# 
# 
# mytest <- apapbicmet.frame %>%
#   # mutate(test = list(prd_data(times, pars))) %>%
#   # mutate(test = list(normL2(prd_data, data, NULL))) %>%
#   # mutate(test = test(pars)) %>%
#   mutate(test = list(obj(pars))) %>%
#   .$test %>%
#   # .[[1]] %>%
#   # head(1)
#   {.}
...
# mytest %>% head(1)
# 
# checkout_hypothesis(apapbicmet.frame, 1, suffix = "wup")
# 
# mycondwise <- obj_condition_wise(obj_datawup, parswup)
# mycondwise
# 
# condwise_sig <- obj_condition_wise(obj_data_sigwup, parswup)
# condwise_sig
# data %>% filter(study %>%  str_detect("Taher"))
# data %>% filter(simulation %>% str_detect("lim"))
# 
# getConditions(prdwup) %>% str_subset("Taher")
# 
# prdwup(timeswup, parswup, conditions = "Taheri2013_NaN_74.9_0_0_0_0_0_10_149.8_0_30", deriv = F)
```
