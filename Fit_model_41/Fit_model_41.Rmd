...
prior_frame2 <- apapbicmet_frame %>% 
  ...
  mutate(
    constr = map(seq_along(x), function(i) {
      constraintL2(prior_mu, sigma = i+0.5) }), 
    hypothesis = paste0("prior_sigma=", 1:4+0.5)
  ) %>% 
  rowwise() %>% 
...
#     rowwise
# 
#   },  machine = c(paste0("knecht", c(1, 2,3)), "ruprecht1"), input = "prior.frame2", filename = "2018_05_07_22_49_fit_job_runbg")
...
# fit_job$purge()
# fit <- readRDS("2018_05_07_22_49_fit_job_results.rds")
```
...
```{r, dependson="setup"}
# apapbicmet_frame <- readRDS("2018_05_04_14_09_apapbicmet.frame.rds")
# # 
# apapbicmet_frame %>%
#   add_column(reactions = list(reactions),)
# 
# 
# saveShiny_dMod.frame(apapbicmet_frame, projectname = "LiMaXBasicModel") # This doesn't work
```
