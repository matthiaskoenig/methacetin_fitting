...
some_na <-
  ...
  select(-sna, -nna) %>% 
  as.data.frame() %>% 
  fitErrorModel(factors = fitErrorModel_factors, plotting = F, blather = T) 
...
P_CO2F <- y_dmod["P_CO2Fc13"] %>% str_replace("R_PDB", "0")

observables_bic <- c(recovery_bic, P_CO2Fc13 = P_CO2F y_dmod[c("DOB")])
