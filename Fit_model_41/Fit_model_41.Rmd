...
# saveRDS(no_mohr.frame, tpaste0("fits_no_mohr_broad_sampling.rds")) # "2018_04_01_20_34_fits_no_mohr_broad_sampling.rds"
...
# Profile Likelihood with three different settings on no_mohr.frame
...
Even with more parameters fixed: The profiles are still wiggly
Jens is saying: There has to be a problem with the numerics
```{r}
...
#   insert("x~-2", x = s0)
# 
# p_sig <- P(trafo_sigma_set, modelname = "p_apapbicmet_sigma_set_2", compile = F)
# compile(p_sig, output = "p_apapbicmet_sigma_set")
saveRDS(p_sig, "p_sig_2.rds")
p_sig <- readRDS("p_sig_2.rds")
```
...
prior.frame_weak <- rbind(prior.frame_weak, prior.frame_weak) %>% 
  mutate(obj = map(c(1:2), function(i) {
    {obj[[i]] + constraintL2(prior_mu, sigma = i*5)}
  })) %>% 
...
profiles_weak_prior <- readRDS("2018_04_13_12_56_profiles_weak_prior_job_results.rds")
```


```{r}
prior.frame_weak <- profiles_weak_prior$ruprecht1
prior.frame_weak$obj %>% walk(loadDLL)
...
```{r}
c(prior.frame$profiles, prior.frame_weak$profiles)%>% plotProfile() +
  coord_cartesian(ylim = c(-2, 4)) + 
  # scale_color_brewer() +
  geom_blank()
# ggplotly()
