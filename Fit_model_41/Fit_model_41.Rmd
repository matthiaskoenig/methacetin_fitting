...
prior_frame <- bind_rows(prior_frame, prior_frame2)
```


```{r}
prior_frame$profiles %>% setNames(prior_frame$hypothesis) %>%  plotProfile() +
  coord_cartesian(ylim = c(-0.5,4))
```

## Profile steps of prior strength 1 and 1.5

```{r}

# walk(myframe$obj, loadDLL)
# profile_steps_job <- map(1:2, function(hypothesis) {
#   myframe <- assign("myframe", prior_frame[c(4,7)[hypothesis], drop = F], pos = .GlobalEnv)
#   runbg({
#     ncores <- detectFreeCores()
#     assign("ncores", ncores, pos = .GlobalEnv)
# 
...
#       assign("fit_obj", obj[[i]], pos = .GlobalEnv)
# 
#       steps <- getSteps(parframes[[i]], tol = 1, nsteps = 3)
...
# 
# 
#   },  machine = c(paste0("knecht", c(4,5)[hypothesis])), input = "myframe", filename = paste0("2018_05_11_13_40_profile_steps_job_runbg_", hypothesis))
# })
# saveRDS(profile_steps_job, file = "2018_05_11_13_40_profile_steps_job.rds")
# profile_steps_job <- readRDS("2018_05_11_13_40_profile_steps_job.rds")
# map(profile_steps_job, . %>% {.$check()})
# wait_for_runbg(profile_steps_job)
# profile_steps_job_results <- map(profile_steps_job, . %>% {.$get()})
# saveRDS(profile_steps_job_results, file = "2018_05_11_13_40_profile_steps_job_results.rds")
# map(profile_steps_job, . %>% {.$purge()})
# profile_steps_job_results <- readRDS("2018_05_11_13_40_profile_steps_job_results.rds") %>% map(1) %>% do.call(rbind,.)
```

```{r}
# myframe <- readRDS("2018_05_11_13_40_profile_steps_job_results.rds") %>% map(1) %>% do.call(rbind,.)
# prior_frame <- full_join(prior_frame, myframe[c("hypothesis", "profile_steps")], by = "hypothesis")
# git_add_dMod.frame(prior_frame)
```

```{r}
prior_frame[["profile_steps"]][[4]] %>% .[[1]] %>% 
  c(prior_frame[["profile_steps"]][[7]] %>% .[[1]] %>% setNames(.,paste0(names(.), "_2"))) %>% 
  plotProfile() +
  coord_cartesian(ylim = c(-.5,4))
```

```{r}
plotPaths(prior_frame$profiles[[4]], whichPar = "KBO_RELCO2")

plotPaths(prior_frame$profiles[[4]], whichPar = "CYP1A2MET_KM_MET")

plotPaths(prior_frame$profiles[[4]], whichPar = "CYP1A2MET_CL")
```


```{r}
myframe <- prior_frame
plotResiduals.dMod.frame <- function(myframe, hypothesis = 1, split = "condition") {
  
  plotResiduals(parframe = myframe[["parframes"]][[hypothesis]], 
                x = myframe[["prd"]][[hypothesis]],
                data = myframe[["data"]][[hypothesis]] + myframe[["data_sig"]][[hypothesis]],
                errmodel = myframe[["e"]][[hypothesis]],
                split = split)
}
```
