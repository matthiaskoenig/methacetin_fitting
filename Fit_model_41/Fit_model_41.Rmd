...
#   # mutate(sigma = sqrt(exp(0)^2 + (value * exp(-3))^2))  %>% #Barstow and Krumbiegel
#   as.datalist
# 
# 
# 
...
trafo_sigma_NA <-
  ...
  insert("x~0", x = str_subset(getSymbols(mytrafo[[i]]), "^S0"), conditionMatch = "(Krumbiegel|Barstow)") %>% 
  insert("x~-3", x = str_subset(getSymbols(mytrafo[[i]]), "^SREL")) %>% 
  {.}
...
apapbicmet_frame <- apapbicmet_frame %>% 
  ...
  mutate(prd_data = list(prd),
         ...
         Also, data_data refers to the data without errors and data to the whole data.
         
         Furthermore, in this hypothesis, all error-parameters in the error model are fixed so that the error bars optically look ok, so that the data points provide some information. I do this, because before I had problems with fitting.
If the fitting is more stable again, I can set them free again, maybe,.
         ")
...
```{r check if the fixed error models are somewhat sensible}
# checkout_hypothesis(apapbicmet_frame, 1, "wup")
# 
# mypred <- (wupprd)(wuptimes, wuppars, deriv = F)
# 
# mypred2 <- (wupe_sig*wupprd)(wuptimes, wuppars, deriv = F)
# myplot2 <- plotPrediction(mypred2) %>% 
#   attr("data") %>% 
#   rename(value_sig = value)
# 
# plotPrediction(mypred) %>% 
#   attr("data") %>% 
#   left_join(myplot2) %>% 
#   
#   filter(!str_detect(name, "(MVe)")) %>% 
#   filter(!(str_detect(name, "P_C")&str_detect(condition, "Krum"))) %>% 
# 
#   ggplot(aes(time, value, color = condition, fill = condition)) + 
#   facet_wrap("name", scales = "free") + 
#   geom_line() +
#   geom_ribbon(mapping = aes(ymin = value-value_sig, ymax = value + value_sig)) + 
#   
...
prior_frame[["hypothesis"]] <- "prior_sigma ="

prior_sigmas <- c(10^(-3:1), seq(0.5,5, by = 0.5)) %>% unique() %>% sort
...
prior_frame <- map(1:length(prior_sigmas), function(i)
  prior_frame %>% 
...
```{r}
# insert_runbg("fit_prior_frame", "fit", "prior_frame")
# fit_prior_frame <- runbg({
...
#       assign("fit_studyname", paste0("fits", digest::digest(hypothesis[[i]])), pos = .GlobalEnv)
#       mstrust(objfun = fit_obj, center = fit_pars, studyname = fit_studyname, sd = 3,
#              blather = F, cores = ncores, fits = 10*ncores) })) %>%
#     rowwise
# 
#   },  machine = c(paste0("ruprecht", 1:2)), input = "prior_frame", filename = "2018_05_17_17_51_fit_prior_frame_runbg"
#     , recover = T
# )
# fit_prior_frame$check()
# runbg(wait_for_runbg(fit_prior_frame), filename = "wait", input = "fit_prior_frame")
# fit_prior_frame_results <- fit_prior_frame$get()
# saveRDS(fit_prior_frame_results, file = "2018_05_17_17_51_fit_prior_frame_results.rds")
# fit_prior_frame$purge()
fit_prior_frame <- readRDS("2018_05_17_17_51_fit_prior_frame_results.rds")
```


```{r}
prior_frame$fits <- fit_prior_frame %>% 
  map("fits") %>% 
  transpose() %>% 
  map(. %>% Reduce(c.parlist,.)) %>% 
  {.}

prior_frame <- prior_frame %>% appendParframes()
```

```{r}
# insert_runbg("profile_steps", "profile_steps", "prior_frame", 1,3)
# profile_steps <- runbg({
#     ncores <- detectFreeCores()
#     assign("ncores", ncores, pos = .GlobalEnv)
# 
#    prior_frame %>%
#     ungroup %>%
#     mutate(profile_steps = map(seq_along(x), function(i) {
#       assign("fit_obj", obj[[i]], pos = .GlobalEnv)
# 
#       steps <- getStepIndices(parframes[[i]], tol = 1, nsteps = 3) %>% setNames(.,.)
#       lapply(steps, function(step) {
#         assign("fit_pars", parframes[[i]] %>% as.parvec(step), pos = .GlobalEnv)
#         myprof <- profile(obj = fit_obj, pars = fit_pars, whichPar = names(fit_pars), cores = ncores)
#         saveRDS(myprof, tpaste0("profile_hypothesis_",i,"_step_",step,".rds"))
#         myprof
#       }) %>%
#         list
# 
...
# 
# 
#   },  machine = c(paste0("knecht", 2)), input = "prior_frame", filename = "2018_05_18_19_53_profile_steps_runbg"
#     , recover = T
# )
# profile_steps$check()
# runbg(wait_for_runbg(profile_steps), filename = "wait", input = "profile_steps")
# profile_steps_results <- profile_steps$get()
# saveRDS(profile_steps_results, file = "2018_05_18_19_53_profile_steps_results.rds")
# profile_steps$purge()
profile_steps <- readRDS("2018_05_18_19_53_profile_steps_results.rds")
```

```{r}
profile_steps$knecht2$profile_steps %>% map(1) %>% map(1) %>% plotProfile() + coord_cartesian(ylim = c(-0.5,4))
ggsave("profiles.pdf")
...
# insert_runbg("refit_condwise", "fit", "prior_frame")

# refit_condwise <- runbg({
#     ncores <- detectFreeCores()
#     assign("ncores", ncores, pos = .GlobalEnv)
# 
#    prior_frame %>%
#     ungroup %>%
#     mutate(condwise_refits = map(seq_along(x), function(i) {
#       assign("fit_obj", obj[[i]], pos = .GlobalEnv)
#       assign("fit_pars", parframes[[i]], pos = .GlobalEnv)
#       assign("fit_studyname", paste0("fits", hypothesis[[i]]), pos = .GlobalEnv)
#       
#       myconditions <- getConditions(obj[[i]]) %>% setNames(.,.)
#       lapply(myconditions, function(cond) {
#         mstrust(objfun = fit_obj, center = fit_pars, studyname = fit_studyname, sd = 3,
#              blather = F, cores = ncores, conditions = cond) %>% as.parframe
#       })
#     })) %>%
#      rowwise
#    
#   },  machine = c(paste0("knecht", 5)), input = "prior_frame", filename = "2018_05_20_12_28_refit_condwise_runbg"
#     , recover = T
# )
# refit_condwise$check()
# runbg(wait_for_runbg(refit_condwise), filename = "wait", input = "refit_condwise")
# refit_condwise_results <- refit_condwise$get()
# saveRDS(refit_condwise_results, file = "2018_05_20_12_28_refit_condwise_results.rds")
# refit_condwise$purge()
refit_condwise <- readRDS("2018_05_20_12_28_refit_condwise_results.rds")
```
...
# insert_runbg("refit_obswise", "fit", "prior_frame")
...
#    prior_frame %>%
#     ungroup %>%
#     mutate(obswise_refits = map(seq_along(x), function(i) {
#       assign("fit_obj", obj[[i]], pos = .GlobalEnv)
#       assign("fit_pars", parframes[[i]], pos = .GlobalEnv)
#       assign("fit_studyname", paste0("fits", hypothesis[[i]]), pos = .GlobalEnv)
#       
#       obs <- data[[i]] %>% as.data.frame() %>% .$name %>% unique
#         
#       myconditions <- map(obs, function(ob) {
#         data[[i]] %>% 
#         as.data.frame()%>% 
#         filter(name %in% ob) %>% 
#         .$condition %>% 
...
#      rowwise
#    
#   },  machine = c(paste0("knecht", 4)), input = "prior_frame", filename = "2018_05_20_12_29_refit_obswise_runbg"
#     , recover = T
# )
# refit_obswise$check()
# runbg(wait_for_runbg(refit_obswise), filename = "wait", input = "refit_obswise")
# refit_obswise_results <- refit_obswise$get()
# saveRDS(refit_obswise_results, file = "2018_05_20_12_29_refit_obswise_runbg_results.rds")
# refit_obswise$purge()
refit_obswise <- readRDS("2018_05_20_12_29_refit_obswise_runbg_results.rds")
```




# Analyse refit-results


### Reconcile results of obswise and condwise refits

```{r}
refit_obswise <- readRDS("2018_05_20_12_29_refit_obswise_runbg_results.rds")
refit_condwise <- readRDS("2018_05_20_12_28_refit_condwise_results.rds")

prior_frame <- refit_condwise$knecht5
prior_frame$obswise_refits <- refit_obswise$knecht4$obswise_refits
```
...
ncond <- length(prior_frame$condwise_refits[[1]])
nobs <- length(prior_frame$obswise_refits[[1]])
```


```{r plot refit values}
pdf(file = "plotValues_hyp_cn.pdf", width = 16, height = 9)

map(1:nhyp, function(hyp) {
  map(1:ncond, function(cn) {
    prior_frame$condwise_refits[[hyp]][[cn]] %>% plotValues() +
      ggtitle(paste0(prior_frame$hypothesis, ", condition: ", names(prior_frame$condwise_refits[[hyp]])[[cn]]))
  })
})

dev.off()

pdf(file = "plotValues_cn_hyp.pdf", width = 16, height = 9)
...
pdf(file = "plotValues_hyp_obs.pdf", width = 16, height = 9)

map(1:nhyp, function(hyp) {
  map(1:nobs, function(cn) {
    prior_frame$obswise_refits[[hyp]][[cn]] %>% plotValues() + 
            ggtitle(paste0(prior_frame$hypothesis, ", observable: ", names(prior_frame$obswise_refits[[hyp]])[[cn]]))
...
pdf(file = "plotValues_obs_hyp.pdf", width = 16, height = 9)
map(1:nobs, function(cn) {
  map(1:nhyp, function(hyp) {
    prior_frame$obswise_refits[[hyp]][[cn]] %>% plotValues() +
      ggtitle(paste0(prior_frame$hypothesis, ", observable: ", names(prior_frame$obswise_refits[[hyp]])[[cn]]))
...
```{r plot refit pars}
# pdf(file = "plotPars_hyp_cn.pdf", width = 16, height = 9)
# 
# map(1:nhyp, function(hyp) {
#   map(1:ncond, function(cn) {
#     prior_frame$condwise_refits[[hyp]][[cn]] %>% plotPars() +
#       ggtitle(paste0(prior_frame$hypothesis, ", condition: ", names(prior_frame$condwise_refits[[hyp]])[[cn]]))
#   })
# })
# 
...
pdf(file = "plotPars_cn_hyp.pdf", width = 16, height = 9)
...
pdf(file = "plotPars_hyp_obs.pdf", width = 16, height = 9)

map(1:nhyp, function(hyp) {
  map(1:nobs, function(cn) {
    prior_frame$obswise_refits[[hyp]][[cn]] %>% plotPars() + 
            ggtitle(paste0(prior_frame$hypothesis, ", observable: ", names(prior_frame$obswise_refits[[hyp]])[[cn]]))
...
pdf(file = "plotPars_obs_hyp.pdf", width = 16, height = 9)
map(1:nobs, function(cn) {
  map(1:nhyp, function(hyp) {
    prior_frame$obswise_refits[[hyp]][[cn]] %>% plotPars() +
      ggtitle(paste0(prior_frame$hypothesis, ", observable: ", names(prior_frame$obswise_refits[[hyp]])[[cn]]))
...
```{r plot single best values condition_wise}
mydf <- map(1:nhyp, function(hyp) {
  map(1:ncond, function(cn) {
    tibble(cond = cn, 
           condition = names(prior_frame$condwise_refits[[hyp]])[[cn]], 
           value = prior_frame$condwise_refits[[hyp]][[cn]]$value[[1]])
  }) %>% do.call(rbind, .) %>% 
      add_column(hyp = hyp, hypothesis = prior_frame$hypothesis[[hyp]])
}) %>% do.call(rbind,.)

mydf %>% 
  mutate(pr_sig = str_replace_all(hypothesis, "prior_sigma = ", "") %>% as.numeric %>% log10()) %>% 
  ggplot(aes(x = pr_sig, y = value)) + 
...
```{r plot single best values obs_wise}

mydf <- map(1:nhyp, function(hyp) {
  map(1:nobs, function(cn) {
    observables <- prior_frame$data[[hyp]] %>% as.data.frame() %>% .$name %>% unique
    
    tibble(cond = cn, 
           condition = observables[[cn]], 
           value = prior_frame$obswise_refits[[hyp]][[cn]]$value[[1]])
  }) %>% do.call(rbind, .) %>% 
      add_column(hyp = hyp, hypothesis = prior_frame$hypothesis[[hyp]])
}) %>% do.call(rbind,.)
...
mydf %>% 
  mutate(pr_sig = str_replace_all(hypothesis, "prior_sigma = ", "") %>% as.numeric %>% log10()) %>% 
  ggplot(aes(x = pr_sig, y = log(value))) + 
  geom_point(aes(color = condition)) + 
  # facet_wrap("condition") +
  geom_blank()
```
...
# Analysis of results
```{r load results and format them for analysis, message=FALSE, warning = FALSE}
# # Load results, prepare the prediction function for plotFluxes
...
# prior_frame <- prior_frame[c(1,3,2,4,22, 5,20,19,6,7,8,9,10,14,11,12,15,13,16,21,17,18,25,23)]
# 
# # Add the result of the profiles without prior, which started from a fit position with prior. 
# # I think it would be better to first refit around the optimum and then start profiling
# profile_without_prior <- readRDS("2018_05_11_20_13_profile_without_prior_results.rds")[[1]]
# prior_frame[4, "profile_wo_prior"] <- list(profile_without_prior$profiles_wo_prior)
# 
# 
# # Rename hypotheses
# prior_frame$hypothesis <- prior_frame$hypothesis %>% str_replace_all(c("srel = -1,s0=-2, " = "", " =" = "=")) 
# # Put the rows into correct order
# prior_frame <- prior_frame[c(1L, 2L, 3L, 4L, 7L, 8L, 9L, 10L, 5L, 6L),]
# 
# 
# 
# prior_frame$data_data <- prior_frame$data
# prior_frame <- prior_frame %>% 
#   rowwise %>% 
#   mutate(data = list(data + data_sig))
