...
```{r}
out_job1 <- runbg({mstrust(obj, mypars, "test_fits", result_path = "test_fits", sd = 5, fits = 10, cores = detectFreeCores(), blather = T)}, machine = "localhost")
```
...
ready = F
while (ready == F){
  ready = out_job1$check()
  Sys.sleep(3)
}

# Retrieve fit results from the background process.
out_job1$get()
myfits_1 <- .runbgOutput
myfits_1[[1]] %>% as.parframe()%>% head(20)
```
```{r}
out_job1$purge()
```
