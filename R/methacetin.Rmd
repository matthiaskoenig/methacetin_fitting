...
job4 <- runbg({myfit <- mstrust(objfun = obj, center = pouter, studyname = "methacetin", cores = 15, fits = 100); myfit}, machine = "knecht1", filename = "job4")
...
```{r}
# fits2 <- job2$get()
# myfit2 <- fits2$knecht4

myfit2 %>% as.parframe()

mypred <- (g*x*p)(mytimes*4, myfit2 %>% as.parframe() %>% as.parvec())
plotCombined(mypred, mydatalist, name %in% names(observables))
```
