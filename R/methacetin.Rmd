...
```{r}
# best_fit7 <- myfit7 %>% as.parframe() %>% as.parvec() %>% {names(.) <- names(.) %>% str_replace_all("\\.","");.} %>% {structure(as.numeric(.), names= names(.))}
# # obj7(best_fit7) # somehow there was a problem with passing best_fit as parvec?
# 
# profile_job7 <- runbg({myprofile <- dMod::profile(obj = obj7,
#                                                   pars = best_fit7,
#                                                   whichPar = names(best_fit7),
#                                                   cores = 12); myprofile},
#                       machine = "knecht6", filename = "profile_job7", input = global_env_without(c("mypred", "job", "myfit", "myplot")))
# save(profile_job7, file = "profile_job7.rda")
```

```{r Look at profile7 - Fit didnt work out}
# load("profile_job7.rda")
# profile_7 <- profile_job7$get()$knecht6
# save(profile_7, file = "profile_7.rda")
# plotProfile(profile_7)
```
...
#                                                   cores = 12); myprofile},
#                       machine = "knecht1", filename = "profile_job_9", input = global_env_without(c("mypred", "job", "myfit", "myplot")))
...
# myplot_e1
# obj_e1(myfit_e1 %>% as.parframe() %>% as.parvec %>% {names(.) <- names(.) %>% str_replace_all("\\.","");.})
...
# myplot_e2
# obj_e2(myfit_e2 %>% as.parframe() %>% as.parvec %>% {names(.) <- names(.) %>% str_replace_all("\\.","");.})
# save(list = ls(), file = "workspace.rda")
...
Two hypotheses
* Increase of CLrenal_apap* fixes it
* multiplying a rate constant to Ave_apap_ -> A_lu_apap_ fixes it.

```{r}
reactions %>% filter(rate %>% str_detect("HLM"))

reactions %>% filter(from %>% str_detect("Ave"))

observables
```

## Free CLrenal to be fitted

```{r Job_CLrenal only error model in dynamic model and no scaling}
load("methacetin.rda")
...
free_parameters_CLrenal <- c("APAPGLU_HLM_CL",  # Vmax value
                     ...
                     "Ka_apap",
                     names(sbml_parameter_descriptions)[names(sbml_parameter_descriptions) %>% str_detect("CLr")]
...
g_CLrenal <- Y(observables_CLrenal, x)#, parameters = c(free_parameters_CLrenal, scale_parameters_CLrenal))  

# err_CLrenal <- Y(error_model_CLrenal, g_CLrenal)

obj_CLrenal <- normL2(mydatalist, (g_CLrenal*x*p_CLrenal))#, errmodel = err_CLrenal)
...
job_CLrenal <- runbg({myfit <- mstrust(objfun = obj_CLrenal, center = pouter_CLrenal, sd = 2, studyname = "methacetin", cores = 16, fits = 100, iterlim = 150); myfit}, machine = "ruprecht2", filename = "job_CLrenal", input = global_env_without(c("mypred", "job", "myfit", "myplot")))
...
myodemodel_to_lung$extended %>% attr("eq") %>% names()
dput(free_parameters_to_lung)
free_parameters_e1
```


```{r Job_to_lung only error model in dynamic model and no scaling}
...
g_to_lung <- Y(observables_to_lung, x)#, parameters = c(free_parameters_to_lung, scale_parameters_to_lung))  
