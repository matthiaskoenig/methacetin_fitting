...
```{r}
old_pars <- readRDS("fit_short_roecker.rds")$parframes[[1]] %>% as.parvec() %>% unclass %>% c(Ka_metc13  = 6.0966305)
...
## Look at data again
```{r}
wup <- obj_condition_wise(model$obj_data[[1]], model$pars[[1]])
wup %>% as.data.frame()
```


```{r}
obs_sig<- data_sig$name %>% unique()
cond_sig <- data_sig %>% as.datalist() %>% names

data_full %>% 
  filter(name == obs_sig[1]) %>% 
```
