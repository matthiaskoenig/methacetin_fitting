...
# Read data
data_full <-
  read_tsv("../../data/limax_pkpd_v45_data.csv") %>%
  select(-model) %>% 
...
free_pars <- c("Kp_co2c13", "KBO_REL_CO2", "KBO_FIX_CO2", "KLI_REL_CO2", "KLI_FIX_CO2", "KLU_EX_CO2", "KLU_EXKM_CO2", "Ka_co2c13", "Ka_metc13", "APAPD_HLM_CL", "APAPD_Km_apap", "Ka_apap")
...
compile(g,x, p, p_sigma_NA, p_co2_0, p_sigma_NA_co2_0, cores = 3, output = "gxp")
remove_c_and_o()
```
...
model <- dMod.frame(
  hypothesis = c("Model v45", "Model v45, co2 liver pool = 0"),
  x = x,
  g = g,
  p = c(p, p_co2_0),
  data = data,
  ...
  x_sig = x,
  g_sig = g,
  p_sig = list(p_sigma_NA, p_sigma_NA_co2_0),
...
test_dMod.frame(model,2)
model$pars[[1]] %>% `[<-`(., 1:length(.), -8) %>%  print_r.named_vector()
```
...
fit_45_job <- runbg({
   ...
   model %>%
    ...
    rowwise
}, machine = c(paste0("knecht", 5), paste0("ruprecht",1:2), "fermi"), input = "model", filename = "2018_08_03_15_36_fit_45_job"
  , recover = T
)
# fit_45_job$check()
# fit_45 <- fit_45_job$get()
# fit_45 %>% str1
# fit_45 <- fit_45[-2] %>% uniteFits %>% appendParframes
# saveRDS(fit_45, file = "fit_45.rds")
# model <- readDMod.frame("fit_45.rds")
# fit_45_job$purge()
```
...
```{r}
# pdf("waterfalls.pdf")
plotValues(model, 1, value < 20000)
plotValues(model, 1, value < 14000)
plotPars(model, 1, tol = 1, nsteps = 3)


plotValues(model, 2, value < 18800)
plotValues(model, 2, value < 12200)
plotPars(model, 2, value < 12200, nsteps = 3, tol = 1)
# dev.off()
```


```{r}
plotCombined(model, 2, 1, !str_detect(name, "ap"), !condition%in%cond_apap, aesthetics = list(color = "study"))
```

```{r force prediction through point}
force_data <- datalist("Fuller2000_C13_76.7_0_0_0_0_0.79_10_0_0_10" = 
                         data.frame(name = "cum_rec_co2c13",
                                    time = 7,
                                    value = 85,
                                    sigma = 0.01,
                                    stringsAsFactors = F))
...
mymodel <- model[2,] %>% 
  rowwise() %>% 
  mutate(data_force = list(force_data),
         obj_force = list(normL2(data_force, prd)),
         obj = list(obj + obj_force), 
         pars = list(as.parvec(parframes,1))) %>% 
  mutate(fit = list(trust(obj, pars, 1,10)))

with(mymodel %>% unlist(F), {
  pred <- prd(times, fit$argument, deriv = F)
  plotCombined(pred, data,!str_detect(name, "apap"), !condition%in%cond_apap, aesthetics = list(color = "study"))
})
```

```{r}
as.pinner <- as.parvec.tbl_df <- function(x,hypothesis = 1, index = 1) {
  pouter <- dMod:::as.parvec.parframe(x[["parframes"]][[hypothesis]], index = index)
  x[["p"]][[hypothesis]](pouter)
}

as.parvec.tbl_df <- function(x,hypothesis = 1, index = 1) {
  dMod:::as.parvec.parframe(x[["parframes"]][[hypothesis]], index = index)
}
model %>% as.parvec(1,1) %>% .[names(.) != "KBO_FIX_CO2"]
```
...
# model <- readDMod.frame("fit_45.rds")
# 
profile_45_job <- runbg({   ncores <- detectFreeCores()
    assign("ncores", ncores, pos = .GlobalEnv)
   ...
   model %>%
    ...
    mutate(profiles = map(seq_along(x), function(i) {
      ...
      steps <- 1
      if (i == 1)
        steps <- c(1, 67, 314, 727)
      if (i == 2)
        steps <- c(1, 143, 200, 655, 841)

      lapply(steps, function(step) {
        mypars <- parframes[[i]] %>% as.parvec(step)
        fixed <- mypars["KBO_FIX_CO2"]
        mypars <- mypars[names(mypars) != "KBO_FIX_CO2"]

        assign("fit_pars", mypars, pos = .GlobalEnv)
        assign("fit_fixed", fixed, pos = .GlobalEnv)

        prof_0 <- profile(obj = fit_obj,
                pars = fit_pars,
                whichPar = names(fit_pars),
                fixed = fit_fixed,
                cores = ncores)
        prof_1 <- profile(obj = fit_obj,
                pars = c(fit_pars, fit_fixed),
                whichPar = names(fit_fixed),
                cores = ncores)

        return(list(prof_0, prof_1))
      }) %>%
        setNames(as.character(steps)) %>%
        list

       })) %>%
      rowwise
}, machine = c(paste0("knecht", 5)), input = "model", filename = "2018_08_03_20_57_profile_45_job"
  , recover = T
)
# profile_45_job$check()
# profile_45 <- profile_45_job$get()
# profile_45 <- profile_45$knecht5 
# profile_45 <- profile_45 %>% 
#   mutate(profiles = unlist(profiles, recursive = F) %>% map(bind_rows) %>% list)
# saveRDS(profile_45, file = "profile_45.rds")
model <- readDMod.frame("profile_45.rds")
# profile_45_job$purge()
```
...
# Write parameters
The dMod.frame "fit_44.rds" gives rise to the plots.pdf and plots_individual.pdf.
...
model %>% plotPars(1, nsteps = 1)
model %>% plotPars(2, nsteps = 1)
