...
try(library(methods))
try(library(base))
setwd('~/tmp_f3uew_1_folder')
rm(list = ls())

load('tmp_f3uew.RData')
files <- list.files(pattern = '.so')
for (f in files) dyn.load(f)
.node <- 1
.runbgOutput <- try({
    mstrust(obj, mypars, "test_fits", result_path = "test_fits", sd = 5, fits = 20, cores = detectFreeCores(), blather = T)
})
save(.runbgOutput, file = 'tmp_f3uew_1_result.RData')