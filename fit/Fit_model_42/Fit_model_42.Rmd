...
# Read data
data_full <-
  read_csv("../../data/limax_pkpd_v42_data.csv") %>%
  select(-model) %>% 
...
fit_42_job <- runbg({
   ...
   myframe %>%
    ...
    rowwise
}, machine = c(paste0("ruprecht1", 1:2), "fermi"), input = "myframe", filename = "2018_07_25_18_13_fit_42_job"
  # , recover = T
)
# fit_42_job$check()
# fit_42 <- fit_42_job$get() %>% uniteFits %>% appendParframes() 
# saveRDS(fit_42, file = "fit_42.rds")
# fit_42 <- readDMod.frame("fit_42.rds")
# fit_42_job$purge()
```

```{r}
plotCombined(fit_42,2,1, str_detect(condition, "Krumbiegel"))
plotCombined(fit_42,2,1, str_detect(name, "met"))

plotPars(fit_42, 2,nsteps = 2)
...
data <- (myframe$data[[1]] + myframe$data_sig[[1]]) %>% as.data.frame()
