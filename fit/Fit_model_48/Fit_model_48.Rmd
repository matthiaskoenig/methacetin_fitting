...
```{r}
save.image("workspace2.rda")
load("workspace2.rda"); loadDLL(x)
```
...
model <-  readDMod.frame("model2.rds")
```
...
```{r}
groups <- list(1:3)
fit_48_2_job <- map(unique(groups), function(grp) {
  assign("grp", grp, pos = .GlobalEnv)  
  grp <<- grp
    runbg({
      ...
      nfits <- case_when(grp == 1 ~ 25*ncores,
                         grp == 2 ~ 35*ncores,
                         grp == 3 ~ 50*ncores)
      assign("nfits", nfits, pos = .GlobalEnv)
      ...
      }, machine = c("ruprecht1","ruprecht2","fermi")[grp], input = c("model", "groups", "grp"),  filename = "2018_08_08_16_36_fit_48_2_job"
      , recover = T
    )
})
# map(fit_48_2_job,. %>% .$check())

# fit_48_2_job[[1]]
# fit_48_2 <- map(fit_48_2_job[1],. %>% {.$get()})
# saveRDS(fit_48_2, file = "fit_48_2_distributed.rds")
# load("2018_08_08_16_36_fit_48_2_job_3_result.RData")
# fit_48_2 <- .runbgOutput %>% appendParframes()
# saveRDS(fit_48_2, "fit_48_2.rds")
model <- readDMod.frame("fit_48_2.rds")
# map(fit_48_2_job,. %>% .$purge())
```
...
modelversion <- paste0(48, "_better_parameterization")
...
```{r}
newparf %>% plotValues(1, value < 10000)
newparf %>% plotPars(10, value<12000)

newparf %>% as.data.frame() %>% 
  filter(value < 12000) %>% 
  ggplot(aes(KLI_REL_CO2, KLI_FIX_CO2, color = value)) + geom_point()
```


```{r}
plotCombined(model, 1,65, study %>% str_detect("Bars|Irv"), name %in% "DOB") +
...
map(hypothesis, function(i) {
  ...
  tibble(parname = names(pinner), parvalue = pinner) %>% write_csv(paste0("pars",i,"_step_", step, ".csv"))
})
