...
trafo_sigma_NA <-
  ...
  insert("x~exp(x)", x = .currentSymbols) %>% 
  insert("x~y+r_x", x = c("KBO_FIX_CO2", "KBO_REL_CO2"), y = c("KLI_FIX_CO2", "KLI_REL_CO2")) %>% 
...
fit_48_job <- runbg({
   ...
   model %>%
    ...
    mutate(fits = map(seq_along(x), function(i) {
      ...
      assign("r_pars", str_detect(pars[[i]], "^r_"))
    ...
    rowwise
}, machine = c(paste0("knecht", c(1,2,5)), paste0("ruprecht", 2), "fermi"), input = "model", filename = "2018_08_07_17_36_fit_48_job"
  , recover = T
)
# fit_48_job$check()
fit_48 <- fit_48_job$get()
fit_48 %>% str1
fit_48 <-  .runbgOutput %>% uniteFits %>%  appendParframes
...
free_pars <- c("Kp_co2c13", "KBO_MAX_CO2", "KBO_REL_CO2", "KBO_FIX_CO2", "KLI_MAX_CO2", "KLI_REL_CO2", "KLI_FIX_CO2", "KLU_EX_CO2", "KLU_EXKM_CO2", "Ka_co2c13", "Ka_metc13", "APAPD_HLM_CL", "APAPD_Km_apap", "Ka_apap")
hypothesis <- 1
pars <- model$parframes[[hypothesis]] %>% as.parvec
pinner <- model$p[[hypothesis]](pars)[[1]][free_pars]
tibble(parname = names(pars), parvalue = pars) %>% write_csv("pars.csv")
```
