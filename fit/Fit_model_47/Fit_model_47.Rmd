...
# # fit_47 %>% str1
# fit_47 <-  fit_47[-3] %>% uniteFits %>%  appendParframes
...
checkout_hypothesis(model,1, "a")


res1 <- aprd(atimes, apars)

controls(ax, name = "optionsSens") <- list(method = "lsodes", atol = 1e-12, rtol = 1e-10)
controls(ax_sig, name = "optionsSens") <- list(method = "lsodes", atol = 1e-12, rtol = 1e-10)

res2 <-  ((ag*ax*ap) + (ag_sig*ax_sig*ap_sig))(atimes, apars)

map(seq_along(res1), function(i) res1[[i]]-res2[[i]]) %>% map(. %>%.[!is.na(.)&!is.nan(.)] %>% `^`(2,.) %>% sum %>% sqrt)


```

```{r rebuild dmod.frame with finer integration tolerances}
...
model <-
  ...
  mutate(prd_data = list(prd),
         ...
         data = list(data_data + data_sig),
         description = "Model v47",
         parframesold = list(parframes))


model$parframesold[[1]] %>% nrow
```


```{r}
groups <- 1:3
parf_inds <- sample(1:2100, 2100)

grp <- 1
parf_ind <- NULL

fit_47_fine_job <- map(unique(groups), function(grp) {
    grp <<- grp
    parf_ind <- case_when(grp == 1 ~ parf_inds[1:700],
                          grp == 2 ~ parf_inds[701:1400],
                          grp == 3 ~ parf_inds[1401:2100])
runbg({
    ncores <- detectFreeCores()
    assign("ncores", ncores, pos = .GlobalEnv)
   ...
   out <- model %>%
    ...
    mutate(fits = map(seq_along(x), function(i) {
      ...
      myparframe <- parframesold[[i]][parf_ind]
      assign("fit_pars", myparframe, pos = .GlobalEnv)
      
      assign("fit_fixed", NULL, pos = .GlobalEnv)
      # assign("fit_fixed", fixed[[i]], pos = .GlobalEnv)
      assign("fit_studyname", paste0("fits", hypothesis[[i]], grp), pos = .GlobalEnv)
      
      mstrust(objfun = fit_obj, center = fit_pars, fixed = fit_fixed, studyname = fit_studyname, sd = 3,
             blather = F, cores = ncores, fits = 10*ncores) })) %>%
    rowwise}, machine = c("knecht1","knecht2","fermi")[grp], input = c("model", "groups", "grp", "parf_inds", "parf_ind"),  filename = paste0("2018_08_07_00_13_fit_47_fine_job", grp)
  , recover = T
)
    })
# map(fit_47_fine_job, . %>% {.$check()})
# fit_47_fine <- map(fit_47_fine_job,. %>% {.$get()})
# save(.runbgOutput, file = "runbgOutput.rda")
fit_47_fine <- fit_47_fine %>% map(1)  %>% uniteFits() %>% appendParframes()

saveRDS(fit_47_fine, file = "fit_47_fine.rds")
model <- readDMod.frame("fit_47_fine.rds")
# map(fit_47_fine_job,. %>% {.$purge()})
...
model <-
  ...
  mutate(prd_data = list(prd),
         ...
         data_data = list(data),
         data = list(data_data + data_sig),
         description = "Model v47_fine_integration")
...
modelversion <- paste0(47, "_fine_integration")
which_hypotheses <- c(1)
step <- 1
model$parframes[[1]][step]
```
