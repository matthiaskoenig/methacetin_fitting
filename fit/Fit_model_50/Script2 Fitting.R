...
fit_bic_job <- runbg({
  ...
  out <- model %>%
    ...
    mutate(fits = map(seq_along(x), function(i) {
      ...
      fit_pars <- msParframe(fit_pars, 50*ncores, seed=sample(1:10000,1), sd=2)
...
input = "model", filename = "fit_v50_f3"
# machine = c(paste0("dmod-denbi-", c(1,2,3,4,5,6,7,8))), input = "model", filename = "fit_v50_f2"
