...
conditions_all <- prior.frame$data %>% .[[1]] %>% names
...
```{r}
myplot <- plotCombined(prior.frame,4,1) %>% attributes() 

myplot$data$prediction %>% 
  ggplot(aes(x = time, y = value, color = condition)) + 
  geom_line() +
  facet_wrap("name", scale = "free", nrow = 3)+
  
  geom_point(data = myplot$data$data, size = 0.5)+ 
  geom_errorbar(data = myplot$data$data, aes(ymin = value - sigma, ymax = value + sigma))+
  theme_dMod()+
  
  
  # scale_color_dMod(guide = F) +
  scale_color_hue(guide = F)+
  # guide_colorbar()
  
  ggtitle("") +
  ylab("Concentration [a.u.]") +
  xlab("Time [min]")+
  geom_blank()

ggsave("~/Promotion/Writing/Conferences/2018 05 LiSym Mannheim midterm evaluation/pics/Limax_all.png", width = 4, height = 6)
```
...
prior.frame$profiles[[4]] %>% 
  filter(!str_detect(whichPar, "offset")) %>% 
  list() %>% 
  plotProfile() + 
  coord_cartesian(ylim = c(-.2,4))

ggsave(filename  = "~/Promotion/Writing/Conferences/2018 05 LiSym Mannheim midterm evaluation/pics/Limaxprofiles.png", width = 6, height = 5)
```
