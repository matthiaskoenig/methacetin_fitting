...
model <- map(krumbiegel_weightings, function(i) {
  model$fixed <- fixed[i]
  model$hypothesis <- paste0(model$hypothesis, ", weighting ", i)
  return(model)
}) %>%
  bind_rows()
...
test_dMod.frame <- function(model, hypothesis = 1, ...){
  ...
  print(with(unlist(model[hypothesis,], F), {
    obj(times, pars = pars, fixed = fixed)
  }))
}
   ...
   model %>%
    ...
    mutate(fits = map(seq_along(x), function(i) {
      ...
      mstrust(objfun = fit_obj, center = fit_pars, fixed = fit_fixed, studyname = fit_studyname, sd = 3,
             blather = F, cores = ncores, fits = 20*ncores) })) %>%
    rowwise
}, machine = c(paste0("knecht",  3:6), paste0("ruprecht", 1:2), "fermi"), input = "model", filename = "2018_08_02_18_08_fit_44_job"
  , recover = T1
)
# fit_44_job$check()
# fit_44 <- fit_44_job$get() %>% uniteFits %>% appendParframes()
# saveRDS(fit_44, file = "fit_44.rds")
# model <- readDMod.frame("fit_44.rds")
# fit_44_job$purge()
```

```{r}
model %>% select(hypothesis)
```
...
plotCombined(model,5,1, str_detect(condition, "Krumbiegel"))
plotCombined(model,5,1, str_detect(name, "met"))

plotPars(model, 5,nsteps = 2)
