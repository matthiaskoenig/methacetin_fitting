...
# Read data
data_full <-
  read_tsv("../../data/limax_pkpd_v44_data.csv") %>%
  select(-model) %>% 
...
free_pars <- c("KBO_REL_CO2", "KBO_FIX_CO2", "KLI_REL_CO2", "KLI_FIX_CO2", "KLU_EX_CO2", "KLU_EXKM_CO2", "Ka_co2c13", "Ka_metc13", "APAPD_HLM_CL", "APAPD_Km_apap", "Ka_apap")
...
model <- dMod.frame(
  ...
  # fixed = list(c("S0_CUM_REC_METC13_Krumbiegel" = -6, "SREL_CUM_REC_METC13_Krumbiegel"))
)
...
model <- model %>% 
  ...
  mutate(prd_data = list(prd),
         ...
         description = "Also fit error pars of Krumbiegel")
    ...
    mutate(fits = map(seq_along(x), function(i) {
      ...
      mstrust(objfun = fit_obj, 
              ...
              parlower = c(
                ...
                KLU_EX_CO2                    	=	-5	, # 10
                KLU_EXKM_CO2                  	=	-5	, # 11
                S0_CUM_REC_METC13_Krumbiegel  	=	-7	, # 12
                SREL_CUM_REC_METC13_Krumbiegel	=	-7	  # 13 
              ),
              parupper = c(
                APAPD_HLM_CL                  	=	8	, # 1
                APAPD_KM_APAP                 	=	8	, # 2
                ...
                KLU_EX_CO2                    	=	8	, # 10
                KLU_EXKM_CO2                  	=	8	, # 11
                S0_CUM_REC_METC13_Krumbiegel  	=	0	, # 12
                SREL_CUM_REC_METC13_Krumbiegel	=	0	  # 13 
              )
              
      )
    })) %>%
     rowwise
}, machine = c(paste0("knecht",  c(5)), "fermi"), input = "model", filename = "2018_08_02_18_08_fit_44_job_3"
  , recover = T
)
# fit_44_job_3$check()
# fit_44_3 <- fit_44_job_3$get()
# save(.runbgOutput, file = "runbgOutput.rda")
# fit_44_3 <- fit_44_3[-5] %>% uniteFits %>% appendParframes()
# saveRDS(fit_44_3, file = "fit_44_3.rds")
# fit_44 <- readRDS("fit_44.rds")
# saveRDS(uniteFits(list(fit_44, fit_44_2), "parframes"), "fit_44_combined.rds")
# model <- readDMod.frame("fit_44_combined.rds")
```
...
```{r}
pdf("plot_values1.pdf")
plotValues(model, 1, value < 13000)
plotValues(model, 1, value < 14000)
plotPars(model, 1, index < 100, tol = 1, nsteps = 20)

model$parframes[[2]]
plotValues(model, 2, value < 18800)
plotValues(model, 2, converged == F & value <20000)
plotPars(model, 2, index < 100, nsteps = 5, tol = 20)

plotValues(model, 3, value < 45600)
plotPars(model, 3, index < 100, tol = 20)
dev.off()
```


```{r}
plotCombined(model, 1, 2, !str_detect(name, "apap"), !condition%in%cond_apap, aesthetics = list(color = "study"))
```

```{r force prediction through point}
force_data <- datalist("Fuller2000_C13_76.7_0_0_0_0_0.79_10_0_0_10" = data.frame(name = "cum_rec_co2c13",
                                  time = 7,
                                  value = 85,
                                  sigma = 0.01,
                                  stringsAsFactors = F))
...
mymodel <- model[3,] %>% 
  ...
  mutate(data_force = list(force_data),
         ...
         pars = list(as.parvec(parframes,2))) %>% 
  mutate(fit = list(trust(obj, pars, 1,10, fixed = c(fixed))))
...
which_hypotheses <- c(1:3)
pdf(file = "plots2pdf", width = 16, height = 9)
map(which_hypotheses, function(i) {
  plotCombined(model, i, 3, condition %in% cond_apap & name %in% observables_apap)
})

map(which_hypotheses, function(i) {
  plotCombined(model, i, 3, condition %in% cond_bic & name %in% observables_bic)
})

map(which_hypotheses, function(i) {
  plotCombined(model, i, 3, condition %in% cond_met & name %in% observables_met)
})
dev.off()
```
...
```{r}
# model <- readDMod.frame("fit_44.rds")
