...
# Read data
data_full <-
  read_tsv("../../data/limax_pkpd_v49_data.csv") %>%
  select(-model) %>% 
...
trafo_sigma_NA <-
  ...
  insert("x~-6", x = str_subset(.currentSymbols, "srel_DOB"), conditionMatch = "Roecker") %>% 
...
studies_apap <- data_full %>% filter(simulation %in% c("apap")) %>% .[["study"]] %>% unique
...
cond_apap <- map(all_cond, . %>% str_subset(studies_apap)) %>% do.call(c,.)
cond_bic <- map(all_cond, . %>% str_subset(studies_bic)) %>% do.call(c,.)
cond_met <- map(all_cond, . %>% str_subset(studies_met)) %>% do.call(c,.)

model <- model %>% 
  mutate(cond_apap = list(cond_apap),
         cond_bic = list(cond_bic),
...
print(with(unlist(model[1,], F), {
    obj(times, pars = pars, conditions = c(cond_apap, cond_bic))
  }))
plotCombined(model,1,1,name%in%"DOB", str_detect(condition, "Irv|Bar|Roe"), time < 0.1)
model$pars[[1]] %>% `[<-`(., 1:length(.), 20) %>% sort_by_name() %>%  print_r.named_vector()
```

```{r}
model %>% ungroup %>% mutate(fit_conditions = map(seq_along(x), function(i) {c(cond_apap[[i]], cond_bic[[i]])})) %>% .$fit_conditions
```
...
fit_49_job <- runbg({
   ...
   out <- model %>%
    ...
    mutate(fits = map(seq_along(x), function(i) {
      ...
      fit_fixed <- c(Ka_metc13 = 6.0966305)
      assign("fit_fixed", fit_fixed, pos = .GlobalEnv)
      
      fit_pars <- pars[[i]] * 0
      fit_pars <- fit_pars[names(fit_pars)!= "Ka_metc13"]
      assign("fit_pars", fit_pars, pos = .GlobalEnv)
      
      
      fit_conditions = c(cond_apap[[i]], cond_bic[[i]])
...
}, machine = c(paste0("knecht", c(1,2,4))), input = "model", filename = "2018_08_09_13_05_fit_49_job"
  , recover = T
)
# fit_49_job$check()
# fit_49 <- fit_49_job$get()
# fit_49 %>% str1
# fit_49 <- fit_49 %>% uniteFits %>% appendParframes
# saveRDS(fit_49, file = "fit_49.rds")
model <- readDMod.frame("fit_49.rds")
# fit_49_job$purge()
```
```{r}
...
fixed <- c(Ka_metc13 = 6.0966305)
model$parframes[[1]] <- append_fixed(model$parframes[[1]], fixed)
```
...
}, machine = c(paste0("knecht", c(2))), input = "model", filename = "2018_08_09_13_05_fit_49_2_job"
  , recover = T
)
# fit_49_2_job$check()
# fit_49_2 <- fit_49_2_job$get()
# fit_49_2 %>% str1
# fit_49_2 <- fit_49_2[[1]] %>% appendParframes
# saveRDS(fit_49_2, file = "fit_49_2.rds")
model <- readDMod.frame("fit_49_2.rds")
# fit_49_2_job$purge()
```
...
Even though there is quite some spread, consider the first 300 Fits to be the same step, since all parameters are well confined except for the max'es and the KLU which have a linear relationship.
```{r}
parframes_summary(model)
```

```{r}
model$parframes[[1]][300:325]
```

```{r}
plotPars(model$parframes[[1]],50, value < 80000)# + geom_point(aes(shape = factor(value)) )
```

```{r}
checkout_hypothesis(model,1)
i <- 1
newparf <- map(1:nrow(parframes), function(i) {
  pars <- parframes %>% as.parvec(i)
  pinner <- p(pars)[[1]][free_pars]
  myp <- pinner %>% unclass() %>% t %>% log
  parframes[i] %>% as.data.frame() %>% .[1:4] %>% cbind(myp) %>% 
    `attr<-`("metanames", attr(parframes, "metanames")) %>% 
    `attr<-`("parameters", colnames(myp)) %>% 
    `attr<-`("class", class(parframes))
}) %>% do.call(rbind,.)

```

```{r}
newparf %>% plotValues(1)
newparf %>% plotValues(1, value<186485)

newparf %>% as.data.frame() %>% 
  filter(value < 80000) %>% 
  ggplot(aes(KLI_MAX_CO2, KBO_MAX_CO2, color = value)) + 
  # geom_point() +
  geom_density2d()
# ggplotly()
...
newparf %>% as.data.frame() %>% 
  filter(value < 80000) %>%
  ggplot(aes(KLU_EX_CO2, KLU_EXKM_CO2, color = value)) + 
  geom_density2d()


mylm <- newparf %>% as.data.frame() %>% 
  filter(value < 80000) %>% 
  lm(KLU_EX_CO2~KLU_EXKM_CO2, data = .)
mylm$coefficients
```


```{r}
newfixed <- model$parframes[[1]] %>% 
  as.data.frame() %>% 
  filter(value < 80000) %>% 
  select(-(1:4)) %>% 
  select(-c(6,9,11,12)) %>% 
...
fit_49_3_job <- runbg({
   ...
   out <- model %>%
    ...
    mutate(fits = map(seq_along(x), function(i) {
      ...
      mclappy(1:nrow(parframes[[i]]), function(j) {
        pars <- parframes[[i]] %>% as.parvec(j)
        fixed
...
}, machine = c(paste0("knecht", c(2))), input = "model", filename = "2018_08_09_13_05_fit_49_3_job"
  , recover = T
)
# fit_49_3_job$check()
# fit_49_3 <- fit_49_3_job$get()
# fit_49_3 %>% str1
# fit_49_3 <- fit_49_3[[1]] %>% appendParframes
# saveRDS(fit_49_3, file = "fit_49_3.rds")
model <- readDMod.frame("fit_49_3.rds")
# fit_49_3_job$purge()
```
...
modelversion <- paste0(49, "Ka_metc13_not_yet_fitted_boundary_for_r_KLI")
...
```{r}
plotPars(model$parframes[[1]],20, value < 75905)# + geom_point(aes(shape = factor(value)) )
```
...
```{r}
plotCombined(model, 1,1, study %>% str_detect("Bars|Irv|Roe"), name %in% "DOB") +
  facet_wrap(c("name", "study"))
```
...
walk(hypothesis, function(i) {
  ...
  tibble(parname = names(pinner), parvalue = pinner) %>% write_csv(paste0("pars",i, modelversion, "_step_", step, ".csv"))
})
