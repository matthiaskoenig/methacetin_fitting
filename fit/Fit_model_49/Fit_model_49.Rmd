...
insert("name ~ value", value = unlist(mget(supplied_pars)), name = supplied_pars) %>% # insert parameters supplied by the data
  ...
  insert("y~(0.1287739 + 0.9990048*x)", x = "KLU_EXKM_CO2", y = "KLU_EX_CO2") %>% 
...
# save.image("workspace1.rda")
load("workspace1.rda"); loadDLL(x)
```
...
```{r}
old_model <- readDMod.frame("fit_49_2.rds")
myparf <- old_model$parframes[[1]]

myparf %>% as.parvec %>% print_r.named_vector()
```

```{r}
fit_pars <-     model$pars[[1]] * 0 + 8
fit_pars[!names(fit_pars)%in%names(fit_fixed)]
```
...
fit_49_3_job <- runbg({
   ...
   out <- model %>%
    ...
    mutate(fits = map(seq_along(x), function(i) {
      ...
      fit_fixed <- c(
          APAPD_HLM_CL 	=	0.79238980400461	, # 1
          APAPD_Km_apap	=	-1.05654353983166	, # 2
          Ka_apap      	=	0.905171392533925	, # 3
          Ka_co2c13    	=	1.77004566984127	, # 4
          KBO_FIX_CO2  	=	0.944164839699419	, # 5
          KBO_REL_CO2  	=	-2.6331453439378	, # 7
          r_KLI_FIX_CO2	=	1.06745666296743	, # 8
          r_KLI_REL_CO2	=	1.26796144594285	, # 10
          Kp_co2c13    	=	0.389162416370634	 ,
          Ka_metc13     = 6.0966305)
...
}, machine = c(paste0("knecht", c(1,2))), input = "model", filename = "2018_08_09_13_05_fit_49_3_job"
  , recover = T
)
# fit_49_3_job$check()
fit_49_3 <- fit_49_3_job$get()
# fit_49_3 %>% str1
# fit_49_3 <- fit_49_3 %>% uniteFits() %>% appendParframes
...
```{r}
mypars <- model$parframes[[1]] %>% as.parvec(1)
mypars %>% print_r.named_vector()
```

```{r}
# # Finally, Fit Ka_metc13
# model <- model %>%
#   ungroup %>%
#   mutate(fits = map(seq_along(x), function(i) {
#     assign("fit_obj", obj[[i]], pos = .GlobalEnv)
...
#       KBO_MAX_CO2  	=	20	, # 1
#       KLI_MAX_CO2  	=	20	, # 2
#       KLU_EXKM_CO2 	=	8.81900658613953	, # 3
#       APAPD_HLM_CL 	=	0.79238980400461	, # 4
#       APAPD_Km_apap	=	-1.05654353983166	, # 5
#       Ka_apap      	=	0.905171392533925	, # 6
#       Ka_co2c13    	=	1.77004566984127	, # 7
#       KBO_FIX_CO2  	=	0.944164839699419	, # 8
#       KBO_REL_CO2  	=	-2.6331453439378	, # 9
#       r_KLI_FIX_CO2	=	1.06745666296743	, # 10
#       r_KLI_REL_CO2	=	1.26796144594285	, # 11
#       Kp_co2c13    	=	0.389162416370634	 # 12
#     )
#     assign("fit_fixed", fit_fixed, pos = .GlobalEnv)
#     
#     fit_pars <- pars[[i]] * 0
...
# saveRDS(model, "fit_49_4.rds")
model <- readDMod.frame("fit_49_4.rds")
...
```{r}
pars <- parframes %>% as.parvec()


pdf("plots Ka_metc13-pdf", width = 20, height = 9)
walk(c(-5,-3,-1,1,3,5), function(i) {
  pars <- parframes %>% as.parvec()
  pars["Ka_metc13"] <- i
  assign("pars", pars, pos = .GlobalEnv)
  p <- plotCombined(prd(times,pars, deriv = F), data, condition %in% cond_met, str_detect(name, "met")) + 
    ggtitle(label = paste("Ka_metc13 = ", i)) 
...
modelversion <- paste0(49, "Ka_metc13 fitted")
...
walk(which_hypotheses, function(i) {
  plotCombined(model, i, step, condition %in% cond_apap & name %in% observables_apap) %>% print
})
