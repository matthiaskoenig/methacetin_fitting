...
```{r}
dMod.frame <- add_column(dMod.frame, g = list(g_apap, g_apap, g_bic, g_met))
```
...
```{r}
dMod.frame <- add_column(dMod.frame, p = list(p_apap, p_apap_free_ka, p_bic, p_met),
...
testframe <- tibble(hypothesis = c("apap", "penner"),
                    ...
                    wupwup = list("moep", diamonds),
                    myfun = list(function(x) {a * x^3}, myfun),
                    a = c(1:2))

checkout_hypothesis(testframe, "apap")
myfun
schwurbel
wupwup
hypothesis
...
checkin_hypothesis <- function(testframe, exclude = NULL) {
  ...
  return(rbind(testframe, as.tibble(new_hypothesis)))
}


hypothesis <- "motherfucker"
checkin_hypothesis(testframe)
```
