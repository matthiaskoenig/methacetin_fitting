...
```{r}
myplot <- analysis_frame[1,] %>% dMf_append_plots(tol =1, ((name %in% "apap")&(str_detect(condition,"Alb")))) %>% .$plot_combined  %>% .[[1]]
myplot + ggtitle(NULL)+ scale_color_dMod(labels = c("Albert1974 Capsule", "Albert 1074 Tablet"))
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/studyspecific/apap_only_Albert_ka_fix.png", width = 10, height = 6)

myplot <- analysis_frame[2,] %>% dMf_append_plots(tol =1, ((name %in% "apap")&(str_detect(condition,"Rawl"))&(str_detect(condition,"(1000)|(500)")))) %>% .$plot_combined %>% .[[1]]

myplot + ggtitle(NULL) + scale_color_dMod(labels = c("Rawlins IV 1000", "Rawlins Oral 1000","Rawlins Oral 500"))
ggsave("~/Promotion/Writing/Presentations/2018 04 03 Peter Jansen LiSyM/Limax/studyspecific/Rawlins.png", width = 10, height = 6)


analysis_frame[1:2,]$best_value
```
...
### Look at the profiles
```{r load workspace}
load("2018_03_26_15_55_workspace_with_profiles_with_fixed_pars.rda")
walk(dMod.frame$x, loadDLL)
```

```{r read profiles with fixe_pars}
# profile_frame_fixed_pars <- profile_job$get()$fermi
# saveRDS(profile_frame_fixed_pars, tpaste0("profile_frame_fixed_pars.rds"))
# save.image(tpaste0("workspace_with_profile_frame_fixed_pars.rda"))
# tpaste0("profile_frame_fixed_pars.rds")
```
In hypothesis Met: The three parameters are identifiable, but the profilin function didn't start from the optimum?
In hypothesis : The profiles just wiggle on the bottom and dont go up. For KLU_EXCO2 and offset_bic there are no profiles plotted, because they are too steep, meaning that those parameters are really well determined.
```{r}
profile_frame_fixed_pars <- readRDS("2018_03_29_17_35_profile_frame_fixed_pars.rds")
profile_frame_fixed_pars[3:4] %>% 
  map(plotProfile)

profile_frame_fixed_pars[[3]] %>% filter(whichPar %>% str_detect("KLU"))
...
### Look at fits
```{r get results (ugly...)}
# bicmet.frame2 <-  readRDS("2018_03_27_13_59_fit_bicmet_job.rds")
# bicmet.frame2 <- bicmet.frame2$get()
# 
# bla <- list.files(pattern = "2018_03_27_13_58") %>% map(load)
# 
# myfits <- map(bicmet.frame2, "fits")
# 
# 
# myfits <- myfits %>% 
#   transpose() %>% 
#   map(~do.call(Reduce, list(f=c.parlist, x = .x))) %>% 
#   Reduce(c.parlist,.)
# 
# # remove(list = "bla")
# 
# # bicmet.frame2 <- bicmet.frame2[[1]][1,1:14]
# 
# bicmet.frame2[["fits"]] <- list(myfits)
# 
# 
# saveRDS(bicmet.frame2, tpaste0("bicmet_refitted.rds"))
```


```{r}
# bicmet.frame2 <- readRDS("2018_03_29_18_09_bicmet_refitted.rds")
walk(bicmet.frame2$x, loadDLL)
```
```{r expand bicmet.frame}
bicmet.frame2 <- 
  bicmet.frame2 %>% 
  dMf_expand_fits() %>% 
  dMf_append_plots()
...
In hypothesis "bicmet, refit around step 1:8": There are some steps, but the best step is once again unique
```{r}
# bicmet.frame2$hypothesis
bicmet.frame2$plot_value
```

* In hypothesis "bicmet, refit around step 1:8": 
    * Step 1 is really different from the others, steps 2-13 only differ significantly in error-pars and Ka_co2c13
    * The parameters CYP1A2_KM_MET and KBO_RELCO2 are really small
    * Some error pars are also relly small
```{r}
bicmet.frame2$parframes[[1]][1:20,] %>% plotPars() +
...
```{r}
fitted_pars <- bicmet.frame2[c("p", "parframes")]

saveRDS(fitted_pars, tpaste0("fitted_pars_bicmet.rds"))
```
